...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return Math.pow(Math.cos(Math.pow(Math.E, x)) + Math.log(Math.pow(1 + y, 2)) + Math.sqrt(Math.pow(Math.E, Math.cos(x)) + Math.pow(Math.sin(Math.PI * z), 2)) + Math.sqrt(1 / x) + Math.cos(Math.pow(y, 2)), Math.sin(z));
    }
